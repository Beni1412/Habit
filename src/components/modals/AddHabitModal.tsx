...
import { X, Plus, Calendar, Zap, Clock, Hash, AlignLeft } from 'lucide-react';

export const AddHabitModal: React.FC = () => {
  const { isAddHabitModalOpen, setIsAddHabitModalOpen, addHabit } = useApp();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0d1c2e]/70 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-[#ffffff] w-full max-w-lg rounded-3xl p-6 sm:p-8 border border-[#bccabb]/40 shadow-2xl space-y-5 relative max-h-[92vh] overflow-y-auto">
        ...
        <div>
          <h2 className="text-2xl font-black text-[#0d1c2e] tracking-tight">Create New Habit</h2>
          <p className="text-xs text-[#6d7b6d] font-medium mt-0.5">
            Add a daily ritual to nourish your companion and boost your growth.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder="e.g. Morning Water, Read 15 Pages, Gym Session"
          ...
          <div className="space-y-1.5">
            <label className="block text-xs font-bold text-[#0d1c2e] uppercase tracking-wide flex items-center gap-1">
              <AlignLeft className="w-3.5 h-3.5" /> Note / Description (Optional)
            </label>
            <input
              type="text"
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="e.g. Drink 2 large glasses right after waking up"
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-center">
              ...
              <span className="text-xs font-black text-[#006d36]">
                +{getPoints(selectedDifficulty)} Leaf Points & +{getPoints(selectedDifficulty) * 3} XP
              </span>
            </div>
          ...
          <button
            type="submit"
            className="w-full py-4 rounded-2xl bg-[#006d36] hover:bg-[#005e2d] text-white font-black text-sm shadow-md transition-all bouncy-button border-b-[#004722] flex items-center justify-center gap-2 mt-2"
          >
            <Plus className="w-5 h-5" /> Save New Habit
          </button>
        </form>
      </div>
    </div>
